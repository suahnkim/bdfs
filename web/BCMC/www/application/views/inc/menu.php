        ...
        <div class="nano-content">
            <nav id="menu" class="nav-main" role="navigation">
                ...
                <ul class="nav nav-main">
                    ...
                    <?php
                    foreach($this->menu->list as $key=>$val){

                        foreach($val as $mainMenuName=>$mainMenu){

                        if(in_array($user->user_auth , $mainMenu['available_account_type'])){
                            ?>
                    <li class="nav-expanded <?php echo(count($mainMenu['sub']) > 0 ? 'nav-active nav-parent' : '');?>"><!--nav-active-->
                        <a class="nav-link" href="<?php echo($mainMenu['v'])?>">
                            <i class="fas <?php echo($mainMenu['icon'])?>" aria-hidden="true"></i>
                            <span><?php echo($mainMenuName)?></span>
                        </a>
                      <?php if(count($mainMenu['sub']) > 0){?>
                        <ul class="nav nav-children">
                            <?php foreach($mainMenu['sub'] as $subMenuName => $subMenu){?>
                            <li>
                                <a class="nav-link" href="<?php echo($subMenu['v'])?>">
