...
class contents_model extends CI_Model {
    ...
    public function getContentsPopularTop($params){
        ...
        $sql = "SELECT t2.* FROM blockchain.CONTENTS_DOWNLOAD_RANK AS t1 LEFT JOIN blockchain.CONTENTS AS t2 ON t1.contents_id = t2.contents_id WHERE t2.state=1 ORDER BY t1.download_cnt DESC LIMIT ". $params->limit;
        $this->key_value = array();
        $this->conn->query($sql , $this->key_value);
        if(!$this->conn->isError()){
            $rows = $this->conn->fetchAll('object');
    ...
    public function setPurchaseDownCountAdd($params){
        if(strlen($params->log_purchase_id) < 1 || strlen($params->accountId) < 1){
            return Utils::customizeResponse(400, 400, "This is not a valid request.", null);
        }

        $sql = "UPDATE LOG_PURCHASE SET down = down + 1 WHERE log_purchase_id = ? AND accountId = ?";
        $this->conn->query($sql, array($params->log_purchase_id, $params->accountId));

        $response = Utils::customizeResponse("200", "200", '다운로드 카운트 추가 완료', "");
