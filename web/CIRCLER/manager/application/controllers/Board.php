...
class Board extends CI_Controller {
    ...
    public function lists(){
        ...
        $this->load->view(MC_VIEWS_PATH."/board/skin/".$board_config->data->skin_name . '/list'  ,array(
            'request_params'          =>  $request_params,
            'data'                          =>  $response->data,
            'paging'                       =>  $paging
        ));
    ...
    public function form(){
        ...
        $this->load->view(MC_VIEWS_PATH."/board/skin/".$board_config->data->skin_name . '/form' ,array(
            'request_params'          =>  $request_params,
            'data'                          =>  $response->data,
        ));
        $this->load->view(COM_VIEWS_PATH."/inc/footer_js");
    ...
    private function _form_submit(){
        ...
        $request_params = (object)array(
            ...
            'secret'           => $this->input->post('secret', true) ? $this->input->post('secret', true) : 0,
            ...
            'fix'                => $this->input->post('fix', true) ? $this->input->post('fix', true) : 0,
    ...
    private function ftpUpload($absolute_path ,  $ftp_make_dir , $filename , $origin_file){
        ...
        $conn_id = ftp_connect(FTP_IP, 7788);
    ...
    public function ftpDelete($file_path , $file_name){
        $result = array();
        $conn_id = ftp_connect(FTP_IP, 7777);
        $conn_login = ftp_login($conn_id, FTP_ID, FTP_PASS);
    ...
    public function download(){
       ...
       $data = $response->data;
        ...
        $url = "http://".$response->data->file_domain  . $response->data->file_path .'/'.$response->data->file_name; 
        $ch = curl_init();
      ...
      $ftp_mak_dir  = date('Y').'/'.date('m').'/'. date('d'); 
			...
			if($_FILES['filename']['error'] == 0) {
					 ...
					 $fileData['file_name'] = time() . '_' . md5($_FILES['filename']['name']) .'.'. $ext;
					 $fileData['file_path'] = '/' .$ftp_mak_dir;
					 $fileData['file_origin_name'] = $_FILES['filename']['name'];
					 $fileData['file_size'] = $_FILES['filename']['size'];
					 $fileData['file_type']  =  $ext;

					 $upfile_result = $this->ftpUpload($absolute_path , $ftp_mak_dir , $fileData['file_name'] , $_FILES['filename']['tmp_name']);
					 if(!$upfile_result['result']){
							 $response = Utils::customizeResponse("200", "302", "업로드에 실패하였습니다.", null);
							 echo json_encode($response);
							 http_response_code($response->status);
