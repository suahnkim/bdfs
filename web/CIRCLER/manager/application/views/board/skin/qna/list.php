...
<table class="sty03">
    <colgroup>
        <col width="10%"/><col width="*"/><col width="25%"/><col width="15%"/><col width="10%"/>
    </colgroup>
    <tr>
        <th>번호</th><th>제목</th><th>이메일 주소</th><th>등록일</th><th>상태</th>
    </tr>
    <?php
      ...
      ?>
    <tr>
        <td><?php echo($num)?></td><td class="left"><?php echo($reply_icon)?><a href="/board/view/<?php echo($val->bbs_id)?>?board_id=<?php echo($val->board_id)?>"><?php echo($val->subject)?></a></td>
        <td><?php echo($val->user_email)?></td>
        <td><?php echo(substr($val->regdate , 0 , 10))?><br/><?php echo(substr($val->regdate , -8))?></td>
        <td><?php if($val->depth == 0){ ?><span class="<?php echo($val->answer == 1)?"":"txt_red"?>"><?php echo($val->answer == 1)?"답변완료":"대기중"?></span><?php } ?></td>
    </tr>
    <?php $num--;}}else{ ?>
