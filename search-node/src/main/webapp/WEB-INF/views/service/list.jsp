...
<script>
	...
	function searchCcontents(nowPage) {
		...
		var url = '/ccsearch/v1/search.do?ccStatus=ready,service&nowPage='+nowPage+'&rowPerPage=' + rowPerPage + '&sortField='+sortField+'&sortOrder='+sortOrder;
		if(contentType != '') {
	...
	function printCContentList(data) {
		...
		for(var i=0; i<result.length; i++) {
			...
			try {
				...
				var lastModify  = fContent['last_modify'];
				if(lastModify == null) {
					lastModify = '';
				}
	...
	function updateCcStatus() {
		...
		if(confirm("본 복합 콘텐츠를 서비스에 반영 하시겠습니까?")) {
	...
	<div id="body_wrap" class="menu_exp">
		...
		<div id="body_boxr">
			...
			<div id="body_sbox">		
				...
				<a href="javascript:updateCcStatus()" class="btn_type_bg btn_type_pink_none_fill" style="position:absolute;right:30px;">서비스 반영</a>
