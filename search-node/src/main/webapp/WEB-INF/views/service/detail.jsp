...
<script>
		...
		for(var i=0; i<metaArr.length; i++) {
			...
			try {
				...
				for(var j=0; j<artworkArr.length; j++) {
					...
					'	<a class="txt-gagam2" title="썸네일 제거" onclick="removePic(this)"><i class="fas fa-minus-circle"></i></a>' +
				...
				var tabHtml = '' +
			        ...
			        '                            <p>'+lastModify+'</p> '+
			        //'                            <a href="" class="btn_type_xs btn_type_blue_none_fill">수정 내용 비교</a> '+
			        '                        </td> '+
			        '                    </tr> '+
			        '                    <tr> '+
			        '                        <td>Country</td> '+
			        ...
			        //'                        <td>Content Price</td> '+
			        //'                        <td class="txt_left"> '+
			        //'                            <input type="text" class="input_type i_size_300" style="margin:0;" value="????원"> '+
			        //'                        </td> '+
			        //'                    </tr> '+
			        '                    <tr> '+
			        '                        <td>Cast</td> '+
	...
	function uploadAddArtwork(mId) {
		if(confirm("해당 정보를 업데이트 하시겠습니까?") == false) {
			return;
		}
	...
	function updateCcStatus(ccid, version, code) {
		...
		var msg = "";
		if(code == 'index') {
			msg = "본 복합 콘텐츠를 서비스 콘텐츠에서 제거하시겠습니까?";
		} else if(code == 'service') {
			msg = "본 복합 콘텐츠를 서비스에 적용시키겠습니까?";
		} else if(code == 'ready') {
			msg = "본 복합 콘텐츠를 서비스 대기 모드로 변경 시키시겠습니까?";
		}
