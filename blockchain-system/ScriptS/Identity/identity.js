...
var accountRouter = require('../ApiS/routes/account.router');

var Log4JS = require('log4js');
var logger ;
Log4JS.configure({
  appenders: { Identitier2: { type: 'file', filename: './logs/identity.log', maxLogSize: 524288, backups: 2, compress: true }
   },
  categories: { default: { appenders: ['Identitier2'], level: 'error' } }
})

logger = Log4JS.getLogger('Identitier2')
logger.level = "debug"
...
var ClaimHolderContractJS = require("./src/contracts/ClaimHolder.js");

///////////////////////////////////


async function startLoomClient() {
    console.log(accountRouter.Tools);
    ...
    CLient = new Client(
        ...
        'default',
        'ws://203.229.154.79:46658/websocket',
        'ws://203.229.154.79:46658/queryws'
    );
    CLient.on('error', err=>{
...
async function getAttributeId(id, attributerId) {
    ...
    // var identityContract = require(IdentityContractJS);
    identityContract = reload(IdentityContractJS);
...
// API configuration
var IdentityContractJS = './identitycontract.js';
var identityContract = require("./identitycontract.js")
...
async function registerId(){
    ...
    await endLoomClient();
    return {"resultCode":0, id: contractId, key: '0x'+toHexString(publicKey) }
}
...
async function addAttribute(targetId, attribute){
    ...
    let issuerAccountTmp = fs.readFileSync(IssuerConfigFile, 'utf8')
    var issuerAccount = JSON.parse(issuerAccountTmp)
