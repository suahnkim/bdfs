...
func getOutPath(req *cmds.Request) string {
	...
	if outPath == "" {
		trimmed := strings.TrimRight(req.Arguments[0], string(os.PathSeparator))
		_, outPath = filepath.Split(trimmed)
		//	[190827-ToM] 경로를 지정하지 않으면 임시 폴더를 사용한다.
		outPath = os.TempDir() + string(os.PathSeparator) + filepath.Clean(outPath)
	}
	return outPath
}
