...
var log = logging.Logger("objectcmd")
...
`,
	...
	Options: []cmdkit.Option{
		...
		cmdkit.BoolOption(allOptionName, "a", ""),
	},
	Run: func(req *cmds.Request, res cmds.ResponseEmitter, env cmds.Environment) error {
		var outLinks []Link
		...
		all, _ := req.Options[allOptionName].(bool)
		if all {
			link, err := api.Object().LinksAll(req.Context, rp.Cid())
			if nil != err {
				return err
			}

			outLinks = make([]Link, len(link.Childs))
			for n, l := range link.Childs {
				outLinks[n] = Link{
					Hash: enc.Encode(l.Cid),
					Name: l.Name,
					Size: l.Size,
					Link: l,
				}
			...
			for iIndex, link := range links {
				outLinks[iIndex] = Link{
	...
	Encoders: cmds.EncoderMap{
		cmds.Text: cmds.MakeTypedEncoder(func(req *cmds.Request, w io.Writer, out *Object) error {
			all, _ := req.Options[allOptionName].(bool)
			if all {
				for iIndex, link := range out.Links {
					fmt.Printf("[%d] %s", iIndex, link.Link.String(""))
				}
			} else {
