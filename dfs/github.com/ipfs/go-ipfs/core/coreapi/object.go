...
/// @fn	(api *ObjectAPI) LinksAll(ctx context.Context, c cid.Cid) (*ipld.Link, error)
/// @brief	입력한 경로(ipfs/{cidv0})의 모든 하위 링크 정보를 검색한다.
/// @return	*ipld.Link	???
/// @return	error	???
/// @param	ctx	???
/// @param	c	???
func (api *ObjectAPI) LinksAll(ctx context.Context, c cid.Cid) (*ipld.Link, error) {
	//	[200708-ToM] 목록 조회 유무를 설정한다.
	link, err := api.dag.GetList(ctx, c, false)
	if nil != err {
		return nil, err
	}
