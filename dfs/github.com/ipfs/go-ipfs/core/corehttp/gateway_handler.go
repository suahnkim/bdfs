...
func (i *gatewayHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" || r.Method == "HEAD" {
		//	[200408-ToM] HTTP I/F 핸들러 함수 변경(ipfs->ccfs)
		i.getOrHeadHandler(ctx, w, r)
		//i.getOrHeadHandler2(ctx, w, r)
		return
	}
...
func (i *gatewayHandler) getOrHeadHandler(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	fmt.Println("getOrHeadHandler():", r.URL)
...
func (i *gatewayHandler) getOrHeadHandler2(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	fmt.Println("getOrHeadHandler2():", r.URL)
	...
	ccid, _ := cid.Decode("QmX81BZDkB65paQuX1sAuBvS7mES7atCTm7gDaPDEHZ6Nm")
	version, _ := cid.Decode("QmX81BZDkB65paQuX1sAuBvS7mES7atCTm7gDaPDEHZ6Nm")
	//	subPaths := []string{"video", "video1.mp4"}
	var subPaths []string

	//	CCFS 내 모든 파일의 링크 정보를 조회한다.
	linkRoot, err := i.api.Object().LinksAll(ctx, version)
	if nil != err {
		webError(w, "failed to Object().LinksAll()", err, http.StatusNotFound)
		return
	}
	...
	if 0 < len(subPaths) {
		if linkRoot, err = mb.FindLinkByPaths(linkRoot.Childs, linkRoot.Cid, subPaths); nil != err {
			webError(w, "failed to mb.FindLinkByPaths()", err, http.StatusNotFound)
			return
		}
	}
	...
	if sList := mb.MakeDownloadChunks(ccid, version, &mLinks); "" != sList {
		//	다운로드 대상 목록을 파일에 기록한다.
		sPath := os.TempDir() + string(os.PathSeparator) + "FileChunks.dat"
		fmt.Println("\n● 청크 목록 파일: ", sPath)
		fmt.Println(sList)
		if err = ioutil.WriteFile(sPath, []byte(sList), os.ModePerm); nil != err {
			webError(w, "청크 목록 작성 실패", err, http.StatusNotFound)
			return
		}
		//	함수 종료 시 임시 파일을 삭제한다.
		...
		pid := "123456789"
		if _, err := mb.OpenChannel(pid, linkRoot, mLinks, sPath); nil != err {
			webError(w, "오프체인 채널 개설 실패", err, http.StatusNotFound)
			return
		}
	}

	dr, err := i.api.Unixfs().GetT(ctx, linkRoot.Cid)
	if nil != err {
		webError(w, "ccfs cat "+escapedURLPath, err, http.StatusNotFound)
		return
	}
	...
	i.addUserHeaders(w) // ok, _now_ write user's headers.
	//w.Header().Set("X-IPFS-Path", urlPath)
	w.Header().Set("X-CCFS-Path", urlPath)
	//	w.Header().Set("Etag", etag)
...
func (i *gatewayHandler) deleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, c, components, err := path.SplitAbsPath(p)
	if err != nil {
