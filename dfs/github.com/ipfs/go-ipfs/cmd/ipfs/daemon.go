...
func daemonFunc(req *cmds.Request, re cmds.ResponseEmitter, env cmds.Environment) (_err error) {
	...
	//	환경 정보를 설정한다.
	mb.SetConfig(&cfg.MediaBlockchain)

	// 오프체인 채널을 초기화한다.
	// [200616-ToM] Sqlite3 DB를 오픈한다.
	if err = mb.InitChannel(cctx.ConfigRoot); nil != err {
		return err
	}

	// 환경 설정의 유효성을 검사한다.
	if err = cfg.MediaBlockchain.Verify(); nil != err {
		return err
	}

	//	온체인 API에 접속 로그인한다.
	if err = mb.Login(); nil != err {
		return err
	}

	//	[200623-ToM] 노드 모니터를 실행한다.
	var storageMax float64 = -1
	//	데이터스토어 최대 설정 크기를 계산한다.
	if length := len(cfg.Datastore.StorageMax); 0 < length {
		storageMax, _ = strconv.ParseFloat(cfg.Datastore.StorageMax[:length-2], 64)
		switch cfg.Datastore.StorageMax[length-2:] {
		case "KB":
			storageMax /= 1024 * 1024
		case "MB":
			storageMax /= 1024
		case "GB":
		case "TB":
	...
	if err = mb.BeginMonitor(cctx.ConfigRoot, storageMax, 8090); nil != err {
		return err
	}
...
func serveHTTPGateway(req *cmds.Request, cctx *oldcmds.Context) (<-chan error, error) {
	...
	var opts = []corehttp.ServeOption{
		...
		//	[200408-ToM] 기존 ipfs -> ccfs 로 변경, ipns 지원 삭제
		corehttp.GatewayOption(writable, "/ipfs", "/ipns"),
		//corehttp.GatewayOption(writable, "/ccfs"),
		corehttp.VersionOption(),
		corehttp.CheckVersionOption(),
